...
Astro es un robot de entrenamiento automatizado asistido por una cámara/coprocesador ESP32-CAM y sensores interactivos. El sistema guía a un perro llamado Oreo mediante comandos de voz pregrabados ("Oreo" y "Rasca") y valida el cumplimiento de las órdenes usando un sensor de proximidad y un sensor de presión. Si Oreo obedece dentro del tiempo límite, recibe un premio de comida de forma automática; si falla, el robot emite un sonido de desaprobación y maneja tiempos de descanso antes de reiniciar el ciclo.
...
- **Dado que** Astro inicia la sesión, **Cuando** reproduce el audio pregrabado con el nombre "Oreo", **Entonces** se activa un temporizador de 2 minutos y el sistema empieza a leer el sensor de proximidad.
- **Dado que** el sensor de proximidad registra que Oreo se ha acercado con éxito dentro de los 2 minutos, **Cuando** se valida la lectura, **Entonces** Astro activa automáticamente el mecanismo para lanzarle un premio de comida, espera 2 minutos en reposo y avanza al siguiente comando.
...
- **Dado que** Oreo rasca con suficiente fuerza el sensor de presión dentro del límite de tiempo, **Cuando** el sistema detecta el impacto, **Entonces** Astro le lanza automáticamente otro premio de comida y finaliza la sesión con éxito.
...
- **Dado que** Astro emitió cualquiera de los dos comandos ("Oreo" o "Rasca"), **Cuando** el temporizador llega a los 2 minutos (Timeout) sin recibir señal del sensor correspondiente (proximidad o presión), **Entonces** Astro debe reproducir un audio de fallo (ej. un silbido o un "oh, qué mal"), NO lanza comida y finaliza el ciclo de entrenamiento.
...
- ¿Qué pasa si el dispensador se bloquea mecánicamente al intentar soltar una de las 5 galletas disponibles?

---

## Requisitos Clave (Must-Haves)

- El sistema **DEBE** integrar un altavoz para reproducir de forma nítida los tres tipos de audio: comandos ("Oreo", "Rasca"), sonidos de éxito y audios de fallo.
...
Sabremos que esto es un éxito si Oreo completa de forma guiada y autónoma los dos comandos, recibiendo sus respectivas recompensas espaciadas por el intervalo de descanso de 2 minutos.

El objetivo es establecer una rutina interactiva de adiestramiento básico con hardware que proporcione retroalimentación clara tanto para el acierto como para el fallo.
